...
const Create = () => {
    ...
    const handleSubmit = (e) => {
        ...
        fetch(`${process.env.REACT_APP_API_URL}.json` , {
            method: 'POST',
            headers:{
 ...
 */}
                        ...
                        <button  className={`${(loading)?'cursor-not-allowed':''} shadow appearance-none bg-primary my-4 text-white rounded p-2 w-full hover:text-primary hover:bg-white transition duration-500 ease-in-out`} type="submit">
                            Submit
                        </button>
                </form>
            </div>
